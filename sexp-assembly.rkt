...
(define-lex-abbrev label
                   (:: (:+ alphabetic) ":"))
(define-lex-abbrev instruction
                   (:: (:+ alphabetic) ":"
(define-lex-abbrev comment
                   (:: "/*" (complement (:: any-string "*/" any-string)) "*/"))

(define-tokens assembly-tokens (label instruction))

(define get-token
  (lexer-src-pos
    [label (token-label lexeme)]
    [instruction (token-instruction (get-token 
    [comment (return-without-pos (get-token input-port))]
    [whitespace (return-without-pos (get-token input-port))]))

(define (assemble port)
  (define (get-tokens port)
    (if (eof-object? (peek-char port))
      '()
      (cons (get-token port) (get-tokens port))))

  (let ([tokens (get-tokens port)])
    tokens))

(displayln (assemble (open-input-string "start: /* test of comments */ ")))
